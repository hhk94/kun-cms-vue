...
export default {
  ...
  mounted() {
    ...
    this.video_item.id = this.$route.params.id
  ...
  methods: {
    ...
    async get_video_detail() {
      ...
      try {
        this.loading = true
        result = await Video.get_video_detail(data)
      } catch (e) {
        this.loading = false
        console.log(e)
      }
      if (result.state == window.g.SUCCESS_STATE) {
        ...
        this.$message.success(result.msg)
        console.log(result)
        this.video_item = result.data
        this.imageUrl = result.data.video_img_url
      } else {
        this.loading = false
        this.$notify({
          title: '提示',
